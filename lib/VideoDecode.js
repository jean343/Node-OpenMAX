...
function VideoDecode(options) {
  ...
  this.inputPortBufferEnabled = false;
...
VideoDecode.prototype._write = function (chunk, enc, next) {
  if (!this.inputPortBufferEnabled) {
    this.inputPortBufferEnabled = true;
    // TODO: keep the state in the component itself
...
VideoDecode.prototype.on('finish', function () {
  var self = this;
  console.log('VideoDecode on finish');
  this.component.emptyBuffer(undefined, function () {
